...
export const TrophyCase: React.FC<{ badges: string[], newBadgeId?: string }> = ({ badges, newBadgeId }) => {
  const allBadges = [
    { id: 'first_step', icon: '👟', name: 'Rookie', desc: 'First Blood' },
    { id: 'on_fire', icon: '🔥', name: 'Ignited', desc: '3 Day Streak' },
    { id: 'champion', icon: '🏆', name: 'Legend', desc: '1000 Pts' },
    { id: 'combo_king', icon: '⚡', name: 'Volt', desc: '4x Combo' },
    { id: 'survivor', icon: '🛡️', name: 'Tank', desc: 'Low HP Win' },
    { id: 'ninja', icon: '🥷', name: 'Ninja', desc: 'Perfect Form' },
  ];

  return (
    <div className="w-full">
        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 flex items-center gap-2">
            <Shield size={10} className="text-yellow-500" /> Agent Accolades
        </h3>
                ...
                return (
                    ...
                    >
                        ...
                        <div className="text-center">
                            <div className="text-[9px] font-black uppercase text-white tracking-wider">{badge.name}</div>
                            <div className="text-[7px] font-medium text-slate-400 leading-none">{badge.desc}</div>
                        </div>
