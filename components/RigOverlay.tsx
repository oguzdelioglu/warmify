...
interface RigOverlayProps {
  ...
  poseLandmarks?: any; // Real-time landmarks from MediaPipe
}
...
const RigOverlay: React.FC<RigOverlayProps> = ({ 
  ...
  useEffect(() => {
    ...
    const drawCyber = (ctx: CanvasRenderingContext2D, joints: any, colors: string[]) => {
        ...
        // 11-13 (Left Arm), 13-15 (Left Forearm), etc.
        ...
        connections.forEach(([startIdx, endIdx]) => {
            ...
            // Outer Glow
            drawLine(ctx, start, end, primary, 6, w, h);
            // Inner Core
    ...
    const getProceduralJoints = (w: number, h: number, tickVal: number) => {
        ...
        // Angles
        let lArmA = Math.PI/2 + 0.3 - armAngle;
        let rArmA = Math.PI/2 - 0.3 + armAngle;
         if (animEx === 'Shadow Boxing') {
             const punch = Math.sin(tickVal * speed * 2);
             lArmA = (punch > 0 ? punch * 0.5 : 0) + 1;
             const punch2 = Math.sin(tickVal * speed * 2 + Math.PI);
             rArmA = Math.PI - ((punch2 > 0 ? punch2 * 0.5 : 0) + 1);
        }
