...
export const SportModeSelector: React.FC<SportModeSelectorProps> = ({ currentMode, onSelectMode, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-[fadeIn_0.3s_ease-out]">
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 rounded-3xl max-w-lg w-full p-6 shadow-2xl animate-[slideUp_0.4s_ease-out]">
                <div className="text-center mb-6">
                    <h2 className="text-3xl font-black text-white mb-2">{t('mode.select_title')}</h2>
                    <p className="text-slate-400 text-sm">{t('mode.select_desc')}</p>
                </div>
                ...
                <div className="space-y-3 mb-6">
                    {sportModes.map(({ mode, icon, gradient }) => (
                        <button
                            ...
                            className={`w-full p-4 rounded-xl border-2 transition-all flex items-center gap-4 group ${currentMode === mode
                        ...
                        >
                            ...
                            <div className="flex-1 text-left">
                                <h3 className="font-bold text-white text-lg">{t(`mode.${mode.toLowerCase()}.name`)}</h3>
                                <p className="text-slate-400 text-xs">{t(`mode.${mode.toLowerCase()}.desc`)}</p>
                            </div>
                            {currentMode === mode && (
